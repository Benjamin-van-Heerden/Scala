...
object Anagrams extends AnagramsInterface:
  ...
  def wordOccurrences(w: Word): Occurrences = 
    w.toLowerCase.groupBy(x => x).map((k, v) => (k, v.length)).toList.sortBy(x => x(0))
  ...
  def combinations(occurrences: Occurrences): List[Occurrences] = 
    val withZeros = occurrences match
        case Nil => List(Nil)
        case x :: xs =>
            for
                j <- (0 to x(1)).toList
  ...
  def sentenceAnagrams(sentence: Sentence): List[Sentence] = 
    def helper(occurences: Occurrences): List[Sentence] =
        occurences match
            case Nil => List(Nil)
            case _ => for {
                wordSig <- combinations(occurences)
                word <- dictionaryByOccurrences(wordSig)
                rest <- helper(subtract(occurences, wordSig))
            } yield word :: rest
...
@main def test =
    println(Anagrams.dictionary.slice(0, 10))
