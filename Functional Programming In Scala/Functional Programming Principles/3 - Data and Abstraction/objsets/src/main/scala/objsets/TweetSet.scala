...
class NonEmpty(val elem: Tweet, val left: TweetSet, val right: TweetSet) extends TweetSet:
  ...
  def descendingByRetweet: TweetList =
    def traverseCreateDescTweets(current: TweetSet, acc: TweetList = Nil): TweetList =
      if current.isEmpty then acc
      else
        val currMax = current.mostRetweeted
        traverseCreateDescTweets(current.remove(currMax), Cons(currMax, acc))
    def reverse(l: TweetList, acc: TweetList = Nil): TweetList =
      if l.isEmpty then acc
      else reverse(l.tail, Cons(l.head, acc))
    val ascending = traverseCreateDescTweets(this)
    reverse(ascending)
...
object GoogleVsApple:
  ...
  lazy val googleTweets: TweetSet = allTweets.filter(t => 
    t.text.replaceAll("[,:/.]", "").split(" ").intersect(google).length > 0)
  lazy val appleTweets: TweetSet = allTweets.filter(t => 
    t.text.replaceAll("[,:/.]", "").split(" ").intersect(apple).length > 0)
